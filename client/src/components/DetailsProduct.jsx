...
export default function DetailsProduct({
  ...
  useEffect(() => {
    ...
    fetch(`http://swipe-setups.vercel.app/api/product/ratings/${model}`)
  ...
  const addReview = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://swipe-setups.vercel.app/api/product/${model}/ratings`,
        {
          method: 'POST',
          headers: {
      ...
      if (response.ok) {
        ...
        const ratingsResponse = await fetch(
          `http://swipe-setups.vercel.app/api/product/ratings/${model}`
        );
  ...
  const updateReview = async () => {
    ...
    if (ratingIndex !== 1) {
      ...
      try {
        const response = await fetch(
          `http://swipe-setups.vercel.app/api/product/${user}/ratings/${model}`,
          {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
